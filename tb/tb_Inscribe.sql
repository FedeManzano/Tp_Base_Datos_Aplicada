...
DECLARE @CANT_COMISION INT = 7
DECLARE @DNI_ALU_RAN VARCHAR(3) = '' 
...
BEGIN 
    ...
    --BEGIN 
        ...
        DECLARE @NRO_CUATR TINYINT = (SELECT NroCuatri FROM @COMISIONES WHERE ID = @INDEX_COM)
        DECLARE @TURNO CHAR(2) = (SELECT Turno FROM @COMISIONES WHERE ID = @INDEX_COM)

        WHILE @CANT_COMISION > 0
        BEGIN 
            EXEC ddbba.sp_Cadena_Random 1,7,3, @S_RES = @DNI_ALU_RAN OUTPUT
            SELECT @DNI_ALU_RAN
